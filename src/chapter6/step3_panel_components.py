#!/usr/bin/env python3
"""
KRenamer Chapter 6 Step 3: 패널별 컴포넌트 분리
Step 2의 거대한 GUI를 기능별 패널 컴포넌트로 분할

변경 사항:
- 거대한 GUI 클래스 → 기능별 패널 클래스들
- FilePanel: 파일 목록 관리 UI 전담
- OptionsPanel: 리네임 옵션 설정 UI 전담  
- PreviewPanel: 미리보기 표시 UI 전담
- MainGUI: 패널들을 조합하는 조정자 역할

핵심 개념:
- 컴포넌트 기반 설계
- UI 모듈화
- 콜백을 통한 컴포넌트 간 통신
- 조정자 패턴 (Coordinator Pattern)
"""
...
class FilePanel:
    ...
    def __init__(self, parent: tk.Widget, engine: RenameEngine, 
...
class OptionsPanel:
    ...
    def create_widgets(self):
        """위젯 생성"""
        # 기본 옵션 프레임
        basic_frame = ttk.LabelFrame(self.parent, text="기본 리네임 옵션", padding="10")
        ...
        # 고급 옵션 프레임
        advanced_frame = ttk.LabelFrame(self.parent, text="고급 옵션", padding="10")
...
class Step3MainGUI:
    ...
    def __init__(self):
        ...
        # 상태 변수
        self.status_var = tk.StringVar(value="컴포넌트들이 준비되었습니다")
    ...
    def setup_window(self):
        """윈도우 기본 설정"""
        self.root.title("Chapter 6 Step 3: 패널별 컴포넌트 분리")
    ...
    def setup_widgets(self):
        ...
        # 상단: 설명
        info_frame = ttk.LabelFrame(main_frame, text="Step 3: 패널별 컴포넌트 분리", padding="10")
        info_frame.pack(fill=tk.X, pady=(0, 10))
        
        info_text = (
            "이 단계에서는 거대한 GUI를 기능별 컴포넌트로 분리합니다:\n"
            "• FilePanel: 파일 목록 관리 UI 전담\n"
            "• OptionsPanel: 리네임 옵션 설정 UI 전담\n"
            "• PreviewPanel: 미리보기 표시 UI 전담\n"
            "• MainGUI: 컴포넌트들을 조합하는 조정자 역할"
        )
        ...
        # 컴포넌트들 생성 및 배치
        self.file_panel = FilePanel(left_panel, self.engine, self.on_data_changed)
        self.options_panel = OptionsPanel(left_panel, self.engine, self.on_data_changed)
        self.preview_panel = PreviewPanel(right_panel, self.engine)
...
def main():
    """메인 함수"""
    print("KRenamer Chapter 6 Step 3: 패널별 컴포넌트 분리")
    print("=" * 50)
    print("학습 내용:")
    print("• 거대한 GUI를 기능별 컴포넌트로 분할")
    print("• FilePanel: 파일 목록 관리 UI 전담")
    print("• OptionsPanel: 리네임 옵션 설정 UI 전담") 
    print("• PreviewPanel: 미리보기 표시 UI 전담")
    print("• MainGUI: 컴포넌트들을 조합하는 조정자 역할")
    print()
    print("핵심 개념:")
    print("• 컴포넌트 기반 설계")
    print("• UI 모듈화")
    print("• 콜백을 통한 컴포넌트 간 통신")
    print("• 조정자 패턴 (Coordinator Pattern)")
    print()
