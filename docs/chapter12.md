# Chapter 12: PyInstaller .exe 파일 만들기

이번 챕터에서는 KRenamer 프로젝트를 PyInstaller를 사용하여 독립 실행 가능한 .exe 파일로 변환하는 전체 과정을 학습합니다. 사용자가 Python을 설치하지 않고도 실행할 수 있는 배포 가능한 애플리케이션을 만들어보겠습니다.

## 🎯 학습 목표

- **PyInstaller 기본 개념** 이해 및 설치
- **단일 파일 실행파일** 생성 방법
- **GUI 애플리케이션 최적화** 기법
- **다중 플랫폼 빌드** 자동화
- **배포 패키지 구성** 및 **인스톨러 제작**
